...
} from '../../../assets/vnc-resources/core/util/browsers.js';
...
export class VncUI {
  ...
  constructor(
    ...
    this.inhibitReconnect = true;
    this.reconnectCallback = undefined;
    this.reconnectPassword = undefined;
  ...
  initFullscreen(): void {
    ...
    if (fullScreenButton === null || fullScreenButton === undefined) {
      return;
    }
  ...
  setupSettingLabels(): void {
    ...
    if (labels) {
      for (let i: number = 0; i < labels.length; i++) {
        ...
        let htmlFor: string = label.htmlFor;
        if (htmlFor !== '') {
          let elem: any = document.getElementById(htmlFor) as any;
          if (elem) {
            elem.label = label;
          }
        } else {
          // If 'for' isn't set, use the first input element child
          let children: HTMLCollection = label.children as HTMLCollection;
          for (let j: number = 0; j < children.length; j++) {
            let child: any = children[j] as any;
  ...
  initRFB(): boolean {
    try {
      ...
      let msg: string = 'Unable to create RFB client -- ' + exc;
      Log.Error(msg);
  ...
  setMouseButtonListener(element, mouseBtn): void {
    if (element !== undefined && element !== null) {
      element.addEventListener('click', function() {
        this.setMouseButton(mouseBtn);
      });
    }
  }

  addTouchSpecificHandlers(): void {
    ...
    if (
      document.getElementById('noVNC_keyboard_button') !== undefined &&
      document.getElementById('noVNC_keyboard_button') !== null
    ) {
      document.getElementById('noVNC_keyboard_button').addEventListener('click', this.toggleVirtualKeyboard);
    }
    ...
    if (keyboardInput !== undefined && keyboardInput !== null) {
      ...
      keyboardInput.addEventListener('submit', function() {
        return false;
      });
    }
  ...
  addExtraKeysHandlers(): void {
    if (document.getElementById('noVNC_toggle_extra_keys_button')) {
      document.getElementById('noVNC_toggle_extra_keys_button').addEventListener('click', this.toggleExtraKeys);
    }
    if (document.getElementById('noVNC_toggle_ctrl_button')) {
      document.getElementById('noVNC_toggle_ctrl_button').addEventListener('click', this.toggleCtrl);
    }
    if (document.getElementById('noVNC_toggle_alt_button')) {
      document.getElementById('noVNC_toggle_alt_button').addEventListener('click', this.toggleAlt);
    }
    if (document.getElementById('noVNC_send_tab_button')) {
      document.getElementById('noVNC_send_tab_button').addEventListener('click', this.sendTab);
    }
    if (document.getElementById('noVNC_send_esc_button')) {
      document.getElementById('noVNC_send_esc_button').addEventListener('click', this.sendEsc);
    }
    if (document.getElementById('noVNC_send_ctrl_alt_del_button')) {
      document.getElementById('noVNC_send_ctrl_alt_del_button').addEventListener('click', this.sendCtrlAltDel);
    }
  }
  ...
  addXvpHandlers(): void {
    ...
    if (document.getElementById('noVNC_xvp_button')) {
      document.getElementById('noVNC_xvp_button').addEventListener('click', _this.toggleXvpPanel);
    }
  }

  addConnectionControlHandlers(): void {
    if (document.getElementById('noVNC_disconnect_button')) {
      document.getElementById('noVNC_disconnect_button').addEventListener('click', this.disconnect);
    }
    if (document.getElementById('noVNC_connect_button')) {
      document.getElementById('noVNC_connect_button').addEventListener('click', this.connect);
    }
    if (document.getElementById('noVNC_cancel_reconnect_button')) {
      document.getElementById('noVNC_cancel_reconnect_button').addEventListener('click', this.cancelReconnect);
    }
    if (document.getElementById('noVNC_password_button')) {
      document.getElementById('noVNC_password_button').addEventListener('click', this.setPassword);
    }
  }

  addClipboardHandlers(): void {
    if (document.getElementById('noVNC_clipboard_button')) {
      document.getElementById('noVNC_clipboard_button').addEventListener('click', this.toggleClipboardPanel);
    }
    if (document.getElementById('noVNC_clipboard_text')) {
      document.getElementById('noVNC_clipboard_text').addEventListener('focus', this.displayBlur);

      document.getElementById('noVNC_clipboard_text').addEventListener('blur', this.displayFocus);

      document.getElementById('noVNC_clipboard_text').addEventListener('change', this.clipboardSend);
    }
    if (document.getElementById('noVNC_clipboard_clear_button')) {
      document.getElementById('noVNC_clipboard_clear_button').addEventListener('click', this.clipboardClear);
    }
  }

  // Add a call to save settings when the element changes,
  // unless the optional parameter changeFunc is used instead.
  addSettingChangeHandler(name, changeFunc?): void {
  ...
  addSettingsHandlers(): void {
    if (document.getElementById('noVNC_settings_button')) {
      document.getElementById('noVNC_settings_button').addEventListener('click', this.toggleSettingsPanel);
    }
  ...
  addFullscreenHandlers(): void {
    document.getElementById('noVNC_fullscreen_button').addEventListener('click', this.toggleFullscreen);
  ...
  notification(rfb, msg, level, options): void {
    this._statusObs.next(msg + ', Level: ' + level);
  }

  activateControlbar(event?): void {
  ...
  toggleControlbarSide(): void {
    // Temporarily disable animation to avoid weird movement
    let bar: HTMLElement = document.getElementById('noVNC_control_bar');
    bar.style.transitionDuration = '0s';
    bar.addEventListener('transitionend', function() {
      this.style.transitionDuration = '';
    });

    let anchor: HTMLElement = document.getElementById('noVNC_control_bar_anchor');
  ...
  showControlbarHint(show: boolean): void {
    let hint: HTMLElement = document.getElementById('noVNC_control_bar_hint');
    if (show) {
  ...
  dragControlbarHandle(e: Event): void {
    if (!this.controlbarGrabbed) {
      return;
    }

    let ptr: any = getPointerEvent(e);
    ...
    if (!this.controlbarDrag) {
      ...
      let dragThreshold: number = 10 * (window.devicePixelRatio || 1);
      let dragDistance: number = Math.abs(ptr.clientY - this.controlbarMouseDownClientY);
    ...
    let eventY: number = ptr.clientY - this.controlbarMouseDownOffsetY;
  ...
  moveControlbarHandle(viewportRelativeY: number): void {
    let handle: HTMLElement = document.getElementById('noVNC_control_bar_handle');
    let handleHeight: number = handle.getBoundingClientRect().height;
    let controlbarBounds: ClientRect = document.getElementById('noVNC_control_bar').getBoundingClientRect();
    ...
    let newY: number = viewportRelativeY;
    ...
    let parentRelativeY: number = newY - controlbarBounds.top;
    handle.style.transform = 'translateY(' + parentRelativeY + 'px)';
  }

  updateControlbarHandle(): void {
    // Since the control bar is fixed on the viewport and not the page,
    // the move function expects coordinates relative the the viewport.
    if (document.getElementById('noVNC_control_bar_handle')) {
      let handle: HTMLElement = document.getElementById('noVNC_control_bar_handle');
      let handleBounds: ClientRect = handle.getBoundingClientRect();
      this.moveControlbarHandle(handleBounds.top);
    }
  }

  controlbarHandleMouseUp(e): void {
  ...
  controlbarHandleMouseDown(e): void {
    ...
    let handle: HTMLElement = document.getElementById('noVNC_control_bar_handle');
    let bounds: ClientRect = handle.getBoundingClientRect();
  ...
  updateSetting(name, value?): void {
    ...
    if (ctrl.type === 'checkbox') {
      ...
      for (let i: number = 0; i < ctrl.options.length; i += 1) {
  ...
  saveSetting(name): any {
    let val;
    let ctrl: HTMLInputElement | HTMLSelectElement | any;
    if (name === 'resize') {
      ctrl = document.getElementById('noVNC_setting_' + name) as HTMLInputElement;
    } else {
      ctrl = document.getElementById('noVNC_setting_' + name) as HTMLSelectElement;
  ...
  getSetting(name: string): any {
    ...
    if (ctrl !== undefined && ctrl !== null && typeof val !== 'undefined' && val !== null && ctrl.type === 'checkbox') {
  ...
  disableSetting(name: string): void {
    let ctrl = document.getElementById('noVNC_setting_' + name) as any;
    if (ctrl) {
      ctrl.disabled = true;
  ...
  enableSetting(name: string): void {
    let ctrl = document.getElementById('noVNC_setting_' + name) as any;
    if (ctrl) {
      ctrl.disabled = false;
  ...
  updateXvpButton(ver): void {
    let xvpButton = document.getElementById('noVNC_xvp_button');
    if (xvpButton === null || xvpButton === undefined) {
      return;
    }
  ...
  clipboardReceive(rfb, text): void {
  ...
  disconnectFinished(rfb, reason): void {
    ...
    } else if (this.getSetting('reconnect') === true && !this.inhibitReconnect) {
      document.getElementById('noVNC_transition_text').textContent = _('Reconnecting...');
      document.documentElement.classList.add('noVNC_reconnecting');

      let delay: number = parseInt(this.getSetting('reconnect_delay'));
      this.reconnectCallback = setTimeout(this.reconnect, delay);
      return;
    }
  ...
  cancelReconnect(): void {
    if (this.reconnectCallback !== null) {
      clearTimeout(this.reconnectCallback);
      this.reconnectCallback = undefined;
    }
  ...
  setPassword(e): void {
    ...
    if (password) {
      ...
      this.reconnectPassword = password;
    }
    let psw_dlg = document.getElementById('noVNC_password_dlg');
    if (psw_dlg && psw_dlg.classList !== undefined) {
  ...
  updateFullscreenButton(): void {
    ...
    if (
      ...
      documentVar.getElementById('noVNC_fullscreen_button').classList.add('noVNC_selected');
    } else {
      documentVar.getElementById('noVNC_fullscreen_button').classList.remove('noVNC_selected');
    }
  }
  ...
  applyResizeMode(): void {
    ...
    if (screen && this.connected && this.rfb.get_display()) {
      ...
      if (this.resizeMode === 'remote') {
        ...
        let downscaleOnly: boolean = this.resizeMode === 'downscale';
  ...
  updateViewClip() {
    ...
    let size: ScreenSizeModel = this.screenSize();
  ...
  enableDisableViewClip(): void {
    // Disable clipping if we are scaling, connected or on touch
    if (this.resizeMode === 'downscale' || this.resizeMode === 'scale' || isTouchDevice) {
  ...
  updateViewDrag(): void {
    ...
    if (this.rfb.get_display().get_viewport() && this.rfb.get_display().clippingDisplay()) {
      clipping = true;
    }

    let viewDragButton: HTMLInputElement = document.getElementById('noVNC_view_drag_button') as HTMLInputElement;
    if (viewDragButton) {
      if (!clipping && this.rfb.get_viewportDrag()) {
  ...
  keyboardinputReset() {
    let kbi: HTMLTextAreaElement = document.getElementById('noVNC_keyboardinput') as HTMLTextAreaElement;
    if (kbi) {
  ...
  setMouseButton(num) {
    ...
    let blist: number[] = [0, 1, 2, 4];
    for (let b: number = 0; b < blist.length; b++) {
      let button = document.getElementById('noVNC_mouse_button' + blist[b]);
      if (button) {
  ...
  updateDesktopName(rfb, name: string): void {
  ...
  addOption(selectbox, text, value): void {
    if (selectbox !== undefined && selectbox !== null) {
      let optn: HTMLOptionElement = document.createElement('OPTION') as HTMLOptionElement;
      optn.text = text;
      optn.value = value;
  ...
  init(): void {
    // Set up translations
    let LINGUAS: string[] = ['de', 'el', 'nl', 'sv'];
    l10n.setup(LINGUAS);
    try {
      if (l10n.language !== 'en' && l10n.dictionary === undefined) {
        WebUtil.fetchJSON(
          '../../../assets/vnc-resources/app/locale/' + l10n.language + '.json',
        ...
        // Default language=en dictionary=undefined
        this.prime();
      }
    } catch (exc) {}
  }
