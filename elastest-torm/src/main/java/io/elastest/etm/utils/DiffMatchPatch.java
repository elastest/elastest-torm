	...
	public Object[] patchApply(LinkedList<Patch> patches, String text) {
		...
		for (Patch aPatch : patches) {
			...
			if (text1.length() > this.Match_MaxBits) {
				...
				// a monster delete.
				start_loc = matchMain(text, text1.substring(0, this.Match_MaxBits), expected_loc);
				if (start_loc != -1) {
					end_loc = matchMain(text, text1.substring(text1.length() - this.Match_MaxBits),
			...
			if (start_loc == -1) {
				...
				if (text1.equals(text2)) {
					...
					if (text1.length() > this.Match_MaxBits
							&& diffLevenshtein(diffs) / (float) text1.length() > this.Patch_DeleteThreshold) {
							...
							if (aDiff.operation != Operation.EQUAL) {
								...
								if (aDiff.operation == Operation.INSERT) {
									...
									text = text.substring(0, start_loc + index2) + text
											.substring(start_loc + diffXIndex(diffs, index1 + aDiff.text.length()));
								}
							}
							if (aDiff.operation != Operation.DELETE) {
