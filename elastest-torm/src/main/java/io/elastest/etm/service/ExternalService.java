...
public class ExternalService {
	...
	public ExternalJob executeExternalTJob(ExternalJob externalJob) throws Exception {
		...
		try {
			...
			externalJob.setExecutionUrl((etInProd ? "http://" + etPublicHost + ":" + etProxyPort
					: "http://localhost" + ":" + etEtmDevGuiPort) + "/#/projects/" + tJob.getProject().getId()
					+ "/tjob/" + tJob.getId() + "/tjob-exec/" + tJobExec.getId() + "/dashboard");
			externalJob.setLogAnalyzerUrl((etInProd ? "http://" + etPublicHost + ":" + etProxyPort
					: "http://localhost" + ":" + etEtmDevGuiPort) + "/#/logmanager?indexName=" + tJobExec.getId());
	...
	public ExternalProject getExternalProjectByExternalIdAndSystemId(String externalId, String externalSystemId) {
		return this.externalProjectRepository.findByExternalIdAndExternalSystemId(externalId, externalSystemId);
	}
	...
	public ExternalTJob getExternalTJobByExternalIdAndSystemId(String externalId, String externalSystemId) {
	...
	public ExternalTJobExecution modifyExternalTJobExec(ExternalTJobExecution externalTJobExec) {
		if (externalTJobExecutionRepository.findOne(externalTJobExec.getId()) != null) {
	...
	public List<TJobExecutionFile> getExternalTJobExecutionFilesUrls(Long exTJobExecId) throws InterruptedException {
		ExternalTJobExecution exTJobExec = externalTJobExecutionRepository.findById(exTJobExecId);
		return esmService.getExternalTJobExecutionFilesUrls(exTJobExec.getExTJob().getId(), exTJobExecId);
	}
	...
	public List<ExternalTestExecution> getTJobExecTestExecutions(Long tJobExecId) {
		ExternalTJobExecution tJobExec = externalTJobExecutionRepository.findById(tJobExecId);
	...
	public ExternalTestCase getExternalTestCaseByExternalIdAndSystemId(String externalId, String externalSystemId) {
		return this.externalTestCaseRepository.findByExternalIdAndExternalSystemId(externalId, externalSystemId);
	}
