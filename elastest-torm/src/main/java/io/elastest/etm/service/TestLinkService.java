...
public class TestLinkService {
	...
	public TestLinkService(ExternalProjectRepository externalProjectRepository,
			...
			ExternalTJobRepository externalTJobRepository, DockerService2 dockerService) {
			...
			try {
				...
				if (!etPublicHost.equals("localhost")) {
					try {
						...
						SocatBindedPort socatBindedPort = dockerService.bindingPort(testLinkIp, etEtmTestLinkPort,
								etDockerNetwork);
	...
	public TestProject createProject(TestProject project) {
		return this.api.createTestProject(project.getName(), project.getPrefix(), project.getNotes(),
				project.isEnableRequirements(), project.isEnableTestPriority(), project.isEnableAutomation(),
				project.isEnableInventory(), project.isActive(), project.isPublic());
	}
	...
	public TestSuite createTestSuite(TestSuite suite) {
		return this.api.createTestSuite(suite.getTestProjectId(), suite.getName(), suite.getDetails(),
				suite.getParentId(), suite.getOrder(), suite.getCheckDuplicatedName(),
	...
	public TestCase[] getSuiteTestCases(Integer suiteId) {
		TestCase[] cases = null;
		try {
			cases = this.api.getTestCasesForTestSuite(suiteId, true, TestCaseDetails.FULL);
		} catch (TestLinkAPIException e) {
			// EMPTY
		}
		return cases;
	...
	public TestCase[] getPlanTestCases(Integer planId) {
		TestCase[] cases = null;
		try {
			cases = (TestCase[]) ArrayUtils.addAll(cases, this.api.getTestCasesForTestPlan(planId, null, null, null,
					null, null, null, null, null, null, TestCaseDetails.FULL));
		} catch (TestLinkAPIException e) {
			// EMPTY
		}
	...
	public TestCase[] getPlanBuildTestCases(Integer planId) {
		...
		if (builds != null) {
			for (Build currentBuild : builds) {
				try {
					cases = (TestCase[]) ArrayUtils.addAll(cases, this.api.getTestCasesForTestPlan(planId, null,
							currentBuild.getId(), null, null, null, null, null, null, null, TestCaseDetails.FULL));
				} catch (TestLinkAPIException e) {
					// EMPTY
				}
	...
	public TestCase createTestCase(TestCase testCase) {
		return this.api.createTestCase(testCase.getName(), testCase.getTestSuiteId(), testCase.getTestProjectId(),
				testCase.getAuthorLogin(), testCase.getSummary(), testCase.getSteps(), testCase.getPreconditions(),
				testCase.getTestCaseStatus(), testCase.getTestImportance(), testCase.getExecutionType(),
				testCase.getOrder(), testCase.getInternalId(), testCase.getCheckDuplicatedName(),
				testCase.getActionOnDuplicatedName());
	}

	public TestCase[] getPlanBuildTestCases(Integer testPlanId, Integer buildId) {
		TestCase[] cases = null;
		try {
			cases = this.api.getTestCasesForTestPlan(testPlanId, null, buildId, null, null, null, null, null, null,
	...
	public TestCase[] getProjectTestCases(Integer projectId) {
		...
		for (TestSuite currentSuite : this.getProjectTestSuites(projectId)) {
			TestCase[] currentTestCases = this.getSuiteTestCases(currentSuite.getId());
			if (currentTestCases != null) {
				testCase = (TestCase[]) ArrayUtils.addAll(testCase, currentTestCases);
			}
		}
		return testCase;
	}
	...
	public ReportTCResultResponse saveExecution(Execution execution, Integer testCaseId) {
		// Save TestCase Execution in Testlink
		ReportTCResultResponse response = this.executeTest(testCaseId, execution.getTestPlanId(),
				execution.getBuildId(), execution.getNotes(), execution.getStatus());
		...
		ExternalTestCase externalTestCase = this.externalTestCaseRepository
				.findByExternalIdAndExternalSystemId(testCaseId.toString(), this.getSystemId());
	...
	public ReportTCResultResponse executeTest(Integer testCaseId, Integer testPlanId, Integer buildId, String notes,
			ExecutionStatus status) {
		ReportTCResultResponse response = this.api.reportTCResult(testCaseId, null, testPlanId, status, buildId, null,
				notes, null, null, null, null, null, null);
		return response;
	}

	public Execution getLastExecutionOfPlan(Integer testPlanId, Integer testCaseId) {
	...
	public Execution[] getPlanTestCaseExecs(Integer testPlanId, Integer testCaseId) {
		return this.testLinkDBService.getExecsByPlanCase(testCaseId, testPlanId);
	}

	public Execution[] getBuildTestCaseExecs(Integer buildId, Integer testCaseId) {
	...
	public TestPlan createTestPlan(TestPlan plan) {
		return this.api.createTestPlan(plan.getName(), plan.getProjectName(), plan.getNotes(), plan.isActive(),
				plan.isPublic());
	}
	...
	public TestPlan[] getAllTestPlans() {
		...
		try {
			for (TestProject currentProject : this.getProjects()) {
				TestPlan[] projectPlans = this.getProjectTestPlans(currentProject.getId());
	...
	public TestPlan getTestPlanByPlanIdAndProjectId(Integer planId, Integer projectId) {
		TestPlan plan = null;
		try {
	...
	public Build createBuild(Build build) {
		return this.api.createBuild(build.getTestPlanId(), build.getName(), build.getNotes());
	}
	...
	public ExternalProject getExternalProjectByTestProjectId(Integer projectId) {
		String externalSystemId = this.getSystemId();
		return this.externalProjectRepository.findByExternalIdAndExternalSystemId(projectId.toString(),
				externalSystemId);
	}

	public ExternalTJob getExternalTJobByPlanId(Integer planId) {
		String externalSystemId = this.getSystemId();
		return this.externalTJobRepository.findByExternalIdAndExternalSystemId(planId.toString(), externalSystemId);
	}

	public ExternalTestCase getExternalTestCaseByTestCaseId(Integer testCaseId) {
		String externalSystemId = this.getSystemId();
		return this.externalTestCaseRepository.findByExternalIdAndExternalSystemId(testCaseId.toString(),
				externalSystemId);
	}

	public ExternalTestExecution getExternalTestExecByExecutionId(Integer execId) {
		String externalSystemId = this.getSystemId();
		return this.externalTestExecutionRepository.findByExternalIdAndExternalSystemId(execId.toString(),
				externalSystemId);
	}
	...
	public void syncProject(TestProject project) {
		...
		try {
			...
			ExternalProject savedPj = externalProjectRepository.findByExternalIdAndExternalSystemId(
					externalProject.getExternalId(), externalProject.getExternalSystemId());
	...
	public void syncProjectTestPlans(Integer projectId, ExternalProject externalProject) {
	...
	public void syncProjectTestPlan(TestPlan testPlan, ExternalProject externalProject) {
		...
		try {
			...
			ExternalTJob savedTJob = externalTJobRepository.findByExternalIdAndExternalSystemId(
					externalTJob.getExternalId(), externalTJob.getExternalSystemId());
	...
	public void syncProjectTestCase(TestCase testCase, ExternalTJob externalTJob) {
		...
		try {
			...
			ExternalTestCase savedTestCase = externalTestCaseRepository.findByExternalIdAndExternalSystemId(
					externalTestCase.getExternalId(), externalTestCase.getExternalSystemId());
	...
	/* ** Exec ** */
	public void syncTestCaseExecs(Integer testCaseId, ExternalTestCase externalTestCase) {
