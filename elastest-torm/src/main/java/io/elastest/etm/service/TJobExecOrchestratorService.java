...
public class TJobExecOrchestratorService {
	...
	public TJobExecOrchestratorService(DockerService2 dockerService, TestSuiteRepository testSuiteRepo,
			...
			DatabaseSessionManager dbmanager, EsmService esmService, SutService sutService,
			DockerComposeService dockerComposeService, ElasticsearchService elasticsearchService,
	...
	public TJobExecution forceEndExecution(TJobExecution tJobExec) {
		...
		if (tJobExec.getTjob().isExternal()) {
			...
			updateTJobExecResultStatus(tJobExec, TJobExecution.ResultEnum.SUCCESS, resultMsg);
		} else {
			String resultMsg = "Stopped";
			updateTJobExecResultStatus(tJobExec, TJobExecution.ResultEnum.STOPPED, resultMsg);
		}
	...
	private void initTSS(TJobExecution tJobExec, String tJobServices) {
		...
		updateTJobExecResultStatus(tJobExec, TJobExecution.ResultEnum.WAITING_TSS, resultMsg);
			...
			for (ObjectNode service : services) {
				if (service.get("selected").toString().equals(Boolean.toString(true))) {
					...
					updateTJobExecResultStatus(tJobExec, TJobExecution.ResultEnum.STARTING_TSS,
							resultMsg + service.get("name").toString().replaceAll("\"", ""));

					String instanceId = esmService.provisionTJobExecServiceInstanceSync(
							service.get("id").toString().replaceAll("\"", ""), tJobExec);
	...
	private void setTJobExecEnvVars(TJobExecution tJobExec, boolean externalTJob, boolean withPublicPrefix) {
		...
		for (String tSSInstanceId : tJobExec.getServicesInstances()) {
			...
			tJobExec.getEnvVars().putAll(esmService.getTSSInstanceEnvVars(ssi, externalTJob, withPublicPrefix));
		}

		// Get monitoring Env Vars
		tJobExec.getEnvVars().putAll(etmContextService.getTJobExecMonitoringEnvVars(tJobExec));
	}
	...
	public void deprovideServices(TJobExecution tJobExec) {
		...
		if (tJobExec.getServicesInstances().size() > 0) {
			...
			instancesAux = new ArrayList<String>(tJobExec.getServicesInstances());
		} else if (esmService.gettSSIByTJobExecAssociated().get(tJobExec.getId()) != null) {
			logger.debug("Deprovide TJob's TSSs stored in the EsmService");
			instancesAux = new ArrayList<String>(esmService.gettSSIByTJobExecAssociated().get(tJobExec.getId()));
		}

		logger.debug("TSS list size: {}", esmService.gettSSIByTJobExecAssociated().get(tJobExec.getId()).size());
	...
	public String waitForSutInContainer(DockerExecution dockerExec, long timeout) throws Exception {
		...
		if (isDockerCompose) {
			List<Container> containersList = this.dockerService
					.getContainersCreatedSinceId(dockerExec.getAppContainerId());
			this.dockerService.getContainersByNamePrefixByGivenList(containersList, sutPrefix,
					ContainersListActionEnum.ADD);
		} else {
	...
	public String getCurrentExecSutMainServiceName(SutSpecification sut, DockerExecution dockerExec) {
		return dockerService.getSutPrefix(dockerExec) + "_" + sut.getMainService() + "_1";
	}

	public void startSutByDockerImage(DockerExecution dockerExec) throws TJobStoppedException {
		// Create container
	...
	public void startSutByDockerCompose(DockerExecution dockerExec) throws Exception {
		...
		DockerComposeCreateProject project = new DockerComposeCreateProject(composeProjectName, dockerComposeYml,
				envList);
	...
	public String setElasTestConfigToDockerComposeYml(String dockerComposeYml, String composeProjectName,
		...
		try {
			...
			for (HashMap.Entry<String, HashMap> service : servicesMap.entrySet()) {
				// Set Logging
				service = this.setLoggingToDockerComposeYmlService(service, composeProjectName, dockerExec);

				// Set Elastest Network
				service = this.setNetworkToDockerComposeYmlService(service, composeProjectName, dockerExec);
			}

			dockerComposeMap = this.setNetworkToDockerComposeYmlRoot(dockerComposeMap, composeProjectName, dockerExec);
			...
			throw new Exception("Error on setting logging into docker compose yml");
		}
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public HashMap.Entry<String, HashMap> setLoggingToDockerComposeYmlService(HashMap.Entry<String, HashMap> service,
		...
		loggingOptionsContent.put("syslog-address", "tcp://"
				+ dockerService.getContainerIpByNetwork(etEtmLogstashContainerName, elastestDockerNetwork) + ":5000");
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public HashMap.Entry<String, HashMap> setNetworkToDockerComposeYmlService(HashMap.Entry<String, HashMap> service,
			String composeProjectName, DockerExecution dockerExec) {
	...
	public void endComposedSutExec(DockerExecution dockerExec) throws Exception {
		String composeProjectName = dockerService.getSutName(dockerExec);
	...
	public void endSutInContainer(DockerExecution dockerExec) throws Exception {
		...
		if (isDockerCompose) {
			List<Container> containersList = this.dockerService
					.getContainersCreatedSinceId(dockerExec.getAppContainerId());
			this.dockerService.getContainersByNamePrefixByGivenList(containersList, sutPrefix,
					ContainersListActionEnum.REMOVE);
		} else {
	...
	public void endDockbeatExec(DockerExecution dockerExec) {
		dockerService.endContainer(dockerService.getDockbeatContainerName(dockerExec));
	}
