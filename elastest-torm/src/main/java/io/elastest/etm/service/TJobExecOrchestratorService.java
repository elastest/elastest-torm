...
public class TJobExecOrchestratorService {
	...
	public TJobExecOrchestratorService(TJobExecRepository tJobExecRepositoryImpl, DatabaseSessionManager dbmanager,
			TSSService esmService, SutService sutService, AbstractMonitoringService monitoringService,
			EtmContextService etmContextService, UtilsService utilsService,
			ExternalTJobExecutionRepository externalTJobExecutionRepository, PlatformService platformService,
	...
	@Async
	public void execFromExternalJob(TJobExecution tJobExec, boolean integratedJenkins) {
		dbmanager.bindSession();
		tJobExec = tJobExecRepositoryImpl.findById(tJobExec.getId()).get();
		monitoringService.createMonitoringIndex(tJobExec.getMonitoringIndicesList());
		...
		try {
			initSupportServicesProvision(execution, tJobExec.getTjob().getSelectedServices());
			...
			if (execution.isWithSut()) {
				...
				updateExecutionResultStatus(execution, ResultEnum.IN_PROGRESS, resultMsg);
			...
			updateTJobExecResultStatus(tJobExec, ResultEnum.EXECUTING_TEST, resultMsg);
		} catch (Exception e) {
	...
	public Future<Void> executeTJob(TJobExecution tJobExec, String tJobServices) {
		...
		if (tJobExec.isMultiExecutionParent()) { // Parent execution
			resultMsg = "Executing Test";
			updateExecutionResultStatus(execution, TJobExecution.ResultEnum.EXECUTING_TEST, resultMsg);

			for (TJobExecution childExec : tJobExec.getExecChilds()) {
				// If parent not stopped, execute
				if (execsAreStopped.containsKey(tJobExecMapName) && !execsAreStopped.get(tJobExecMapName)) {
			...
			try {
				initSupportServicesProvision(new Execution(tJobExec), tJobServices);
				...
				updateExecutionResultStatus(execution, ResultEnum.EXECUTING_TEST, resultMsg);

				platformService.deployAndRunTJobExecution(execution);

				// Saving test suites
				if (!(execution.getTJobExec().getTestSuites().size() > 0) && execution.getReportTestSuite() != null) {
					logger.debug("Saving test suitest from a TJob on docker");
					etmTestResultService.saveTestResults(execution.getReportTestSuite(), tJobExec);
				...
				if (!"end error".equals(e.getMessage())) {
					...
					updateExecutionResultStatus(execution, ResultEnum.ERROR, resultMsg);
					...
					try {
						...
						logger.error("Error on end TJob Exec {}", tJobExec.getId(), e1);
					}
				}
			} finally {
				if (tJobServices != null && tJobServices != "") {
					try {
						...
						logger.error("TJob Exec {} => Exception on deprovision TSS: {}", tJobExec.getId(),
								e.getMessage());
	...
	private TJobExecution initMultiTJobExecution(TJobExecution tJobExec) {
		try {
			...
			// method)
			for (MultiConfig currentMultiConfig : tJobExec.getMultiConfigurations()) {
			...
			if (numOfChilds > 0) {
				tJobExec = createChildTJobExecs(tJobExec, tJobExec.getParameters(), namesList, valuesList);
			}
	...
	public TJobExecution createChildTJobExecs(TJobExecution parentTJobExec, List<Parameter> parametersList,
			List<String> namesList, List<List<String>> valuesList) {
		...
		if (valuesList.size() > 1) {
			valuesList = new ArrayList<>(valuesList.subList(1, valuesList.size()));
		} else {
			valuesList = new ArrayList<>();
		}

		for (String currentValue : currentValueList) {
			List<Parameter> receivedParametersList = new ArrayList<>(parametersList);
			...
			if (namesList.size() > 0 && currentValueList.size() > 0) {
				parentTJobExec = createChildTJobExecs(parentTJobExec, receivedParametersList, namesList, valuesList);
			}
			// Last
			else {
				TJobExecution childTJobExec = new TJobExecution();
				childTJobExec.setType(TypeEnum.CHILD);
				childTJobExec.setMonitoringStorageType(parentTJobExec.getMonitoringStorageType());
	...
	public TJobExecution forceEndExecution(TJobExecution tJobExec) throws Exception {
		tJobExec = tJobExecRepositoryImpl.findById(tJobExec.getId()).get();
		...
		if (tJobExec.isMultiExecutionParent()) {
			for (TJobExecution childExec : tJobExec.getExecChilds()) {
				...
				updateTJobExecResultStatus(childExec, ResultEnum.STOPPED, resultMsg);
			}
		}
		...
		if (tJobExec.getTjob().isExternal()) {
			...
			updateExecutionResultStatus(execution, ResultEnum.SUCCESS, resultMsg);
		} else {
			String resultMsg = "Stopped";
			updateExecutionResultStatus(execution, ResultEnum.STOPPED, resultMsg);
		}
	...
	public void saveMultiParentFinishStatus(TJobExecution tJobExec, Execution execution) {
		ResultEnum finishStatus = ResultEnum.SUCCESS;

		for (TJobExecution childExec : tJobExec.getExecChilds()) {
			if (childExec.getResult() == ResultEnum.FAIL || childExec.getResult() == ResultEnum.ERROR
	...
	public void endAllExecs(Execution execution, boolean force) throws Exception {
		try {
	...
	public void releaseResourcesFromExtExecution(TJobExecution tJobExec) {
		try {
			...
			logger.error("Exception deprovisioning TSSs associated with an external execution.");
		}
		if (tJobExec.isWithSut()) {
			try {
				...
				logger.error("Error deprovisioning a SUT used from an external execution.");
			}
		}
	...
	public void initSupportServicesProvision(Execution execution, String tJobServices) throws Exception {
		try {
			if (tJobServices != null && tJobServices != "") {
	...
	private void provideServices(String tJobServices, Execution execution) throws Exception {
		logger.info("Start the service provision.");
		String resultMsg = "Starting Test Support Service: ";
		try {
			TJobSupportService[] tssArray = UtilTools.convertJsonStringToObj(tJobServices, TJobSupportService[].class,
					Include.NON_EMPTY);
			List<TJobSupportService> services = Arrays.asList(tssArray);

			// Start EMS first if is selected
			List<TJobSupportService> servicesWithoutEMS = provideEmsTssIfSelected(services, execution);

			for (TJobSupportService service : servicesWithoutEMS) {
				if (service.isSelected()) {
					updateExecutionResultStatus(execution, ResultEnum.STARTING_TSS, resultMsg + service.getName());
	...
	private String provideService(TJobSupportService service, Execution execution) {
		String instanceId = "";
		...
		if (utilsService.isElastestMini()) {
			...
			if (tSSService.isSharedTssInstanceByServiceId(tssId)) {
				...
				tSSService.provisionExecutionSharedTSSSync(tssId, execution, instanceId);
			} else {
				// Else provision async and wait after for tss
				tSSService.provisionExecutionServiceInstanceAsync(tssId, execution, instanceId);
			}

			String serviceName = tSSService.getServiceNameByServiceId(tssId).toUpperCase();

			tSSService.waitForTssStartedInMini(execution, instanceId, serviceName);
		} else { // Sync provision
			instanceId = tSSService.provisionExecutionServiceInstanceSync(tssId, execution);
		}

		logger.debug("{} {} = > Adding TSS instance id {} to execution", execution.getTJobExecType(),
				execution.getExecutionId(), instanceId);

		if (execution.isExternal()) {
			execution.getExternalTJobExec().getServicesInstances().add(instanceId);
		} else {
	...
	public List<TJobSupportService> provideEmsTssIfSelected(List<TJobSupportService> services, Execution execution)
			throws JsonParseException, JsonMappingException, IOException {
		List<TJobSupportService> servicesWithoutEMS = new ArrayList<>(services);
		int pos = 0;
		for (TJobSupportService service : services) {
			if (service.getName().toLowerCase().equals("ems") && service.isSelected()) {
	 ...
	 */
	private Map<String, String> getTJobExecTssEnvVars(boolean externalTJob, boolean withPublicPrefix,
			String tSSInstanceId) {
		SupportServiceInstance ssi = tSSService.getTJobServiceInstanceById(tSSInstanceId);
		Map<String, String> tssInstanceEnvVars = tSSService.getTSSInstanceEnvVars(ssi, externalTJob, withPublicPrefix);
	 ...
	 */
	private Map<String, String> getExternalTJobExecTssEnvVars(boolean publicEnvVars, boolean withPublicPrefix,
			String tSSInstanceId) {
		SupportServiceInstance ssi = tSSService.getExternalTJobServiceInstanceById(tSSInstanceId);
		Map<String, String> tssInstanceEnvVars = tSSService.getTSSInstanceEnvVars(ssi, publicEnvVars, withPublicPrefix);
	...
	private void setExecutionTssEnvVars(Execution execution, boolean withPublicPrefix, String tSSInstanceId) {
		if (execution.isExternal()) {
			this.setExternalTJobExecTssEnvVars(execution.getExternalTJobExec(), withPublicPrefix, tSSInstanceId);
		} else {
			this.setTJobExecTssEnvVars(execution.getTJobExec(), execution.gettJob().isExternal(), false, tSSInstanceId);
		}
	}
	 ...
	 */
	private void setExternalTJobExecTssEnvVars(ExternalTJobExecution exTJobExec, boolean withPublicPrefix,
			String tSSInstanceId) {
		Map<String, String> envVars = new HashMap<>();
		envVars.putAll(exTJobExec.getEnvVars());
		envVars.putAll(this.getExternalTJobExecTssEnvVars(false, withPublicPrefix, tSSInstanceId));
	 ...
	 */
	private void setTJobExecTssEnvVars(TJobExecution tJobExec, boolean externalTJob, boolean withPublicPrefix,
			String tSSInstanceId) {
		Map<String, String> envVars = new HashMap<>();
		envVars.putAll(tJobExec.getEnvVars());
		envVars.putAll(this.getTJobExecTssEnvVars(externalTJob, withPublicPrefix, tSSInstanceId));
		tJobExec.setEnvVars(envVars);
	}

	private void setTJobExecEnvVars(TJobExecution tJobExec, boolean externalTJob, boolean withPublicPrefix)
			throws Exception {
		Map<String, String> envVars = new HashMap<>();
		envVars.putAll(tJobExec.getEnvVars());
		// Get TSS Env Vars
		...
		envVars.putAll(etmContextService.getTJobExecMonitoringEnvVars(tJobExec));
		 ...
		 */
		String attachmentsApiURL = etmContextService.getContextInfo().getEtmInternalApiUrl() + "tjob/exec/"
				+ tJobExec.getId() + "/attachment";
		envVars.put("ET_ETM_TJOB_ATTACHMENT_API", attachmentsApiURL);

		// Generate the SUT name
		String sutContainerName = platformService.generateContainerName(PlatformService.ContainerPrefix.SUT,
		...
		if (envVars.containsKey(etEusApiKey)) {
			...
			if (eusApi != null) {
				...
				EusExecutionData eusExecutionDate = new EusExecutionData(tJobExec, "", useSutNetwork, sutContainerName);
		...
		if (sutExec != null) {
			...
			String sutPublicPort = sutPublicPortLong != null ? sutPublicPortLong.toString() : null;

			// Sut HOST
			String sutHostKey = "ET_SUT_HOST";
			envVars.put(sutHostKey,
					(sutPublicPort != null && !utilsService.isDefaultEtPublicHost())
			...
			if (sutPort != null && !sutPort.isEmpty()) {
				...
				envVars.put(sutPortKey,
						sutPublicPort != null && !utilsService.isDefaultEtPublicHost() ? sutPublicPort : sut.getPort());
		...
		String testContainerName = platformService.generateContainerName(PlatformService.ContainerPrefix.TEST,
				new Execution(tJobExec));
	...
	public void deprovisionServices(Execution execution) {
		...
		logger.info("{} {} => Start the services deprovision.", execType, execId);
		List<String> instancesAux = new ArrayList<String>();

		List<String> servicesInstances = tSSService.getServicesInstancesByExecution(execution);

		if (servicesInstances != null && servicesInstances.size() > 0) {
			logger.debug("{} {} => Deprovisioning TJob's TSSs stored in the TJob object", execType, execId);
			instancesAux = servicesInstances;
		} else if (tSSService.gettSSIByExecutionAssociated(execution).get(execId) != null) {
			logger.debug("{} {} => Deprovisioning TJob's TSSs stored in the EsmService", execType, execId);
			instancesAux = tSSService.gettSSIByExecutionAssociated(execution).get(execId);
		}

		logger.debug("{} {} => TSS list size: {}", execType, execId, instancesAux);
		for (String instanceId : instancesAux) {
			tSSService.deprovisionExecutionServiceInstance(instanceId, execution);
			logger.debug("{} {} => TSS Instance id to deprovision: {}", execType, execId, instanceId);
		}
	}
	...
	public Future<Void> executeExternalTJob(ExternalTJobExecution exec) {
		...
		execution.addObserver(new StatusUpdater());
		try {
			updateExecutionResultStatus(execution, ResultEnum.IN_PROGRESS, "Initializing execution...");

			// TSS
			initSupportServicesProvision(execution, exec.getExTJob().getSelectedServices());
	...
	public Future<Void> endExternalTJob(ExternalTJobExecution exec) {
		dbmanager.bindSession();
		ExternalTJobExecution externalTJobExec = externalTJobExecutionRepository.findById(exec.getId()).get();

		// If withSut, end containers
		if (externalTJobExec.isWithSut() && ResultEnum.isFinishedOrNotExecuted(externalTJobExec.getResult())) {
			...
			try {
				...
				if (sut.getSutType() == SutTypeEnum.DEPLOYED) {
					...
					if (sut.isInstrumentedByElastest() && sut.isInstrumentalized()) {
						logger.debug("TJob Exec {} => Undeploying Beats", execution.getTJobExec().getId());
		...
		if (exec.getExTJob().getSelectedServices() != null && exec.getExTJob().getSelectedServices() != "") {
			try {
				deprovisionServices(new Execution(exec));
			} catch (Exception e) {
				logger.error("TJob Exec {} => Exception on deprovision TSS: {}", exec.getId(), e.getMessage());
				// TODO Customize Exception
			}
		}
	...
	public void initSut(Execution execution, boolean publicSut) throws Exception {
		try {
			SutSpecification sut = execution.getSut();
			...
			// If it's MANAGED SuT
				...
				try {
					...
					if (publicSut || utilsService.isKubernetes()) {
						...
						String sutName = platformService.generateContainerName(ContainerPrefix.SUT, execution);
						ServiceBindedPort socatBindedPortObj = platformService.getBindedPort(sutName, sutName, null,
								execution.getSut().getPort(), null);
						sutExec.setPublicPort(Long.parseLong(socatBindedPortObj.getBindedPort()));
			...
			if (execution.isExternal()) {
				execution.getExternalTJobExec().getEnvVars().put("ET_SUT_URL", sutExec.getUrl());
			}
		} catch (TJobStoppedException e) {
			throw e;
		} catch (Exception e) {
	...
	public SutExecution startSutDeployedOutside(Execution execution) throws Exception {
		SutSpecification sut = execution.getSut();
		SutExecution sutExec;
		String sutIP = "";
		...
		try {
			...
			// Sut instrumented by EIM
			if (sut.isInstrumentedByElastest() && sut.isInstrumentalized()) {
				execution.updateTJobExecutionStatus(ResultEnum.WAITING_SUT, "Deploying beats in Sut");

				if (execution.isExternal()) {
					logger.debug("External TJob Exec {} => Deploy Beats", execution.getExternalTJobExec().getId());
				} else {
					logger.debug("TJob Exec {} => Deploy Beats", execution.getTJobExec().getId());
				}
			...
			throw new Exception("Error preparing the Sut to use in a test: " + e.getMessage() + ".");
		}
	}

	private SutExecution startManagedSut(Execution execution) throws Exception {
		String resultMsg = "Preparing dockerized SuT";
		updateExecutionResultStatus(execution, ResultEnum.EXECUTING_SUT, resultMsg);
		logger.info(resultMsg + " " + execution.getExecutionId());

		SutExecution sutExec = sutService.createSutExecutionBySut(execution.getSut());
		execution.setSutExec(sutExec);
		try {
	...
	public String getMapNameByExec(Execution execution) {
		if (execution.isExternal()) {
			return getMapNameByExternalTJobExec(execution.getExternalTJobExec());
		} else {
			return getMapNameByTJobExec(execution.getTJobExec());
		}
	}

	public String getMapNameByExternalTJobExec(ExternalTJobExecution exTJobExec) {
		return exTJobExec.getExTJob().getId() + "_" + exTJobExec.getId();
	}

	private void startExternalMonitoring(SutSpecification sut, SutExecution sutExec, String key, Date startDate) {
		List<SharedAsyncModel<Void>> sharedExMonitoringDBAsync = new ArrayList<>();

		// External ES for logs
		Future<Void> asyncExElasticsearchForLogs = sutService.manageSutExecutionUsingExternalElasticsearchForLogs(sut,
				sutExec.getSutExecMonitoringIndex(), startDate, asyncExternalMonitoringDBSutExecs, key,
				EXEC_END_DATE_KEY);

		sharedExMonitoringDBAsync.add(new SharedAsyncModel<>(asyncExElasticsearchForLogs));

		// External Prometheus for Metrics
		Future<Void> asyncExPrometheusForMetrics = sutService.manageSutExecutionUsingExternalPrometheusForMetrics(sut,
				sutExec.getSutExecMonitoringIndex(), startDate, asyncExternalMonitoringDBSutExecs, key,
				EXEC_END_DATE_KEY);
		sharedExMonitoringDBAsync.add(new SharedAsyncModel<>(asyncExPrometheusForMetrics));

		// Add Async list to Execs map
		asyncExternalMonitoringDBSutExecs.put(key, sharedExMonitoringDBAsync);
	}

	public void updateManageSutByExtESEndDate(Date endDate, TJobExecution tJobExec) {
		this.updateManageSutByExtESEndDate(endDate, getMapNameByTJobExec(tJobExec));
	}
	...
	public void updateManageSutByExtESEndDate(Date endDate, String mapKey) {
		...
		for (SharedAsyncModel<Void> sharedAsyncModel : asyncExternalMonitoringDBSutExecs.get(mapKey)) {
	...
	public void stopManageSutByExternalElasticsearch(TJobExecution tJobExec) {
		this.stopManageSutByExternalMonitoringDB(getMapNameByTJobExec(tJobExec));
	}

	public void stopManageSutByExternalMonitoringDB(String mapKey) {
		sutService.stopManageSutByExternalMonitoringDB(asyncExternalMonitoringDBSutExecs, mapKey);
	}
	...
	public void updateExecutionResultStatus(Execution execution) {
		if (execution.isExternal()) {
			ExternalTJobExecution externalTJobExec = execution.getExternalTJobExec();
			updateExternalTJobExecResultStatus(externalTJobExec, externalTJobExec.getResult(),
					externalTJobExec.getResultMsg());
		} else {
			TJobExecution tJobExec = execution.getTJobExec();
			updateTJobExecResultStatus(tJobExec, tJobExec.getResult(), tJobExec.getResultMsg());
		}
	}

	public TJobExecution updateTJobExecResultStatus(TJobExecution tJobExec, ResultEnum result, String msg) {
		tJobExec.setResult(result);
		tJobExec.setResultMsg(msg);
		return tJobExecRepositoryImpl.save(tJobExec);
	}

	public void updateExternalTJobExecResultStatus(ExternalTJobExecution externalTJobExec, ResultEnum result,
			String msg) {
	...
	public void updateGenericExecResultStatus(Execution exection, ResultEnum result, String msg) {
		if (exection.isExternal()) {
			updateExternalTJobExecResultStatus(exection.getExternalTJobExec(), result, msg);
		} else {
	...
	protected void saveFinishStatus(TJobExecution tJobExec, Execution execution) {
		...
		if (tJobExec.getTestSuites() != null && tJobExec.getTestSuites().size() > 0) {
