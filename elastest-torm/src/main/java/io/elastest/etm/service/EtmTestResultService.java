...
public class EtmTestResultService {
    ...
    public EtmTestResultService(TestSuiteRepository testSuiteRepo, TestCaseRepository testCaseRepo,
    ...
    public void saveTestResults(List<ReportTestSuite> testSuites, TJobExecution tJobExec) {
        ...
        if (testSuites != null && testSuites.size() > 0) {
            for (ReportTestSuite reportTestSuite : testSuites) {
                ...
                for (ReportTestCase reportTestCase : reportTestSuite.getTestCases()) {
                    ...
                    try {
                        Date startDate = this.monitoringService
                                .findFirstStartTestMsgAndGetTimestamp(tJobExec.getMonitoringIndex(),
                                        tSuite.getName(), tCase.getName(), Arrays.asList("test"));
                        tCase.setStartDate(startDate);

                        Date endDate = this.monitoringService.findFirstFinishTestMsgAndGetTimestamp(
                                tJobExec.getMonitoringIndex(), tSuite.getName(), tCase.getName(),
                                Arrays.asList("test"));
                        tCase.setEndDate(endDate);
                    } catch (Exception e) {
                        logger.debug("Cannot save start/end date for Test Case {}", tCase.getName(),
                                e);
                    }
                ...
                tJobExec.getTestSuites().add(testSuiteRepo.saveAndFlush(tSuite));
            }
        }
    }
