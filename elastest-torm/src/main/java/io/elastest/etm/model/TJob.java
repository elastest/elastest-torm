...
public class TJob {
	...
	@JsonView({ BasicAttTJob.class, BasicAttTJobExec.class, BasicAttProject.class })
	@Column(name = "name")
	@JsonProperty("name")
	...
	public boolean isSelectedService(String serviceName) {
		try {
			...
			for (ObjectNode service : services) {
				...
				if (serviceName.toLowerCase().equals(currentServiceName.toLowerCase())) {
	...
	public List<ObjectNode> getSelectedServicesObj() throws JsonParseException, JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();

		return Arrays.asList(mapper.readValue(selectedServices, ObjectNode[].class));
	}
	...
	public List<ObjectNode> getSupportServicesObj() {
		...
		try {
			ObjectMapper mapper = new ObjectMapper();
			services = Arrays.asList(mapper.readValue(this.getSelectedServices(), ObjectNode[].class));
		} catch (Exception e) {
		}
		return services;
	}
